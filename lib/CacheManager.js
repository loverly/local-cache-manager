...
CacheManager.prototype.setObject = function setObject(key, value, namespace) {
  var hasNameSpace, nameSpacedData, data, cachedObj, objToSet,
...
CacheManager.prototype.setCategoricalSets = function setCategoricalSets(key, setsToSet, namespace) {
  ...
  // if there is dpace in the local cache, keep retrying
  //if (!storageSpace) {
    this._retryCachingCategoricalSets(setsMetaData, namespace); //wrap in a conditional to meet basecase
  //}
};
...
CacheManager.prototype._retryCachingCategoricalSets = function _retryCachingCategoricalSets(setsMetaData, namespace){
  var newKey, key, newValue,
      storageSpace = this.storageSpace,
      collection = this.collection,
      collector = this.collector,
      _this = this;
  ...
  try {
    ...
    this._retryCachingCategoricalSets(setsMetaData);
  }
